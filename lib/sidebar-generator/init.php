...
class Sidebar_Generator {
	...
	public function __construct() {
		$this->prefix  = MYTHEME_CORE_THEME_PREFIX;
		$this->version = MYTHEME_CORE;
		...
		<div class="widgets-holder-wrap">
			<div id="rdtheme-new-sidebar" class="widgets-sortables">
				<div class="sidebar-name">
					...
					<h2><?php esc_html_e( 'Add New Sidebar', 'mytheme-core' ); ?><span class="spinner"></span></h2>
				</div>
				<div class="sidebar-description">
					<form style="padding:0 7px;" method="POST" action="<?php echo esc_url( admin_url( 'admin-ajax.php?action=rdtheme_add_sidebar' ) );?>">
						...
						<table class="form-table">
							<tr>
								<th scope="row"><?php esc_html_e( 'Name', 'mytheme-core' ) ?></th>
								<td><input type="text" class="text" name="name" value=""></td>
								<td><input type="submit" class="button-primary" value="<?php esc_html_e( 'Add', 'mytheme-core' ) ?>"></td>
							</tr>
						</table>
					</form>
				</div>
	...
	public function load_scripts() {
		...
		$localize_data = array(
			'confirm'  => esc_html__( 'Are you sure you want to remove this custom sidebar', 'mytheme-core' ),
			'failed'   => esc_html__( 'Operation failed' , 'mytheme-core' ),
	...
	public function ajax_add_sidebar() {
		...
		if ( empty( $name ) ) {
			wp_send_json_error( esc_html__( "Sidebar name can't be empty", 'mytheme-core' ) );
		}
		if ( empty( $nonce ) ) {
			wp_send_json_error( esc_html__( 'Empty nonce', 'mytheme-core' ) );
		}
		if ( ! wp_verify_nonce( $nonce, 'rdtheme_add_sidebar' ) ) {
			wp_send_json_error( esc_html__( 'Invalid nonce', 'mytheme-core' ) );
		}
		...
		if ( array_key_exists( $id, $sidebars ) ) {
			wp_send_json_error( esc_html__( 'Sidebar with the same name already exists. Please choose a different name', 'mytheme-core' ) );
		}
	...
	public function ajax_remove_sidebar() {
		...
		if ( empty( $id ) ) {
			wp_send_json_error( esc_html__( 'Sidebar ID not found', 'mytheme-core' ) );
		}
		if ( empty( $nonce ) ) {
			wp_send_json_error( esc_html__( 'Empty nonce', 'mytheme-core' ) );
		}
